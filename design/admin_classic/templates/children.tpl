...
{let item_type=ezpreference( 'admin_subitems_limit' )|int
    configured_default_limit_raw=ezini( 'SevenXThemesAdminClassicSettings', 'SubitemsDefaultLimit', 'sevenxthemesadminclassic.ini' )|int
    configured_max_limit_raw=ezini( 'SevenXThemesAdminClassicSettings', 'SubitemsMaxLimit', 'sevenxthemesadminclassic.ini' )|int
    configured_default_limit=cond( gt( $configured_default_limit_raw, 0 ), $configured_default_limit_raw, 200 )
    configured_max_limit=cond( gt( $configured_max_limit_raw, 0 ), $configured_max_limit_raw, 5000 )
    raw_number_of_items=cond( gt( $item_type, 0 ), $item_type, $configured_default_limit )
    number_of_items=min( $raw_number_of_items, $configured_max_limit )
     can_remove=false()
     can_move=false()
     can_edit=false()
     can_create=false()
...
<div class="left">
    <p>
    {def $set_subitems_limit_url_base='/user/preferences/set/admin_subitems_limit/'}
    {'Items per page'|i18n( 'design/admin/node/view/full' )}:
    {if eq( $number_of_items, 200 )}
        <span class="current">200</span>
    {else}
        <a href={concat( $set_subitems_limit_url_base, '200' )|ezurl} title="{'Show 200 items per page.'|i18n( 'design/admin/node/view/full' )}">200</a>
    {/if}
    |
    {if eq( $number_of_items, 500 )}
        <span class="current">500</span>
    {else}
        <a href={concat( $set_subitems_limit_url_base, '500' )|ezurl} title="{'Show 500 items per page.'|i18n( 'design/admin/node/view/full' )}">500</a>
    {/if}
    |
    {if eq( $number_of_items, 1000 )}
        <span class="current">1000</span>
    {else}
        <a href={concat( $set_subitems_limit_url_base, '1000' )|ezurl} title="{'Show 1000 items per page.'|i18n( 'design/admin/node/view/full' )}">1000</a>
    {/if}
    |
    <input id="admin-subitems-limit-input" type="text" value="{$number_of_items}" size="5" title="{'Set a custom number of sub items per page.'|i18n( 'design/admin/node/view/full' )}" />
    <input class="button" type="button" value="{'Set'|i18n( 'design/admin/node/view/full' )}" onclick="var value = parseInt( document.getElementById( 'admin-subitems-limit-input' ).value, 10 ); if ( isNaN( value ) || value < 1 ) value = {$configured_default_limit}; if ( value > {$configured_max_limit} ) value = {$configured_max_limit}; window.location = '{$set_subitems_limit_url_base|wash( javascript )}' + value; return false;" title="{'Apply custom sub items limit for browsing.'|i18n( 'design/admin/node/view/full' )}" />
    {undef $set_subitems_limit_url_base}
    </p>
</div>
<div class="right">
        <p>
