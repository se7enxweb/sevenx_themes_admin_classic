 ...
 *
 * SevenX Themes Admin Classic kernel override.
 * Modifications are clearly marked with:
...
class ezpKernelWeb implements ezpWebBasedKernelHandler
{
    ...
    public function __construct( array $settings = array() )
    {
        ...
        // BEGIN SevenX : SevenXThemesAdminClassic : Kernel Hack
        // Dynamically prepend admin_classic to AdditionalSiteDesignList when the
        // session variable is enabled, giving admin_classic templates priority over
        // the default admin3 design for SSR / non-JS admin content tree rendering.
        $ini  = eZINI::instance();
        $http = eZHTTPTool::instance();

        $sevenxAdminClassicINI = eZINI::instance( 'sevenxthemesadminclassic.ini' );
        $sevenxAdminClassicSessionVarName   = $sevenxAdminClassicINI->variable(
            'SevenXThemesAdminClassicSettings',
            'SwitchAdminDesignSessionVariableName'
        );
        $sevenxAdminClassicAdminSiteaccess  = $sevenxAdminClassicINI->variable(
            'SevenXThemesAdminClassicSettings',
            'SharedAdminSiteaccessName'
        );

        if ( $GLOBALS['eZCurrentAccess']['name'] === $sevenxAdminClassicAdminSiteaccess )
        {
            // Critical for session-driven design switching: design base cache is keyed
            // only by siteaccess name, so it must be disabled or per-session design
            // order changes will not take effect on next page load.
            $ini->setVariable( 'DesignSettings', 'DesignLocationCache', 'disabled' );
            ...
            $isClassicModeEnabled = $http->hasSessionVariable( $sevenxAdminClassicSessionVarName )
                && $http->sessionVariable( $sevenxAdminClassicSessionVarName ) === 'enabled';

            $sessionAdditionalDesignList = null;
            if ( $http->hasSessionVariable( $designListSessionVarName ) )
            {
                $sessionAdditionalDesignList = $http->sessionVariable( $designListSessionVarName );
            }

            if ( $isClassicModeEnabled )
            {
                // Classic mode: keep SiteDesign fixed to admin3 and use classic as
                // additional fallback design in requested order.
                $ini->setVariable( 'DesignSettings', 'SiteDesign', 'admin3' );
                if ( is_array( $sessionAdditionalDesignList ) && !empty( $sessionAdditionalDesignList ) )
                {
                    $ini->setVariable( 'DesignSettings', 'AdditionalSiteDesignList', $sessionAdditionalDesignList );
                }
                else
                {
                    $ini->setVariable( 'DesignSettings', 'AdditionalSiteDesignList',
            ...
            {
                // Admin3 mode: keep admin3 active and remove classic from fallback chain.
                $ini->setVariable( 'DesignSettings', 'SiteDesign', 'admin3' );
                if ( is_array( $sessionAdditionalDesignList ) && !empty( $sessionAdditionalDesignList ) )
                {
                    $ini->setVariable( 'DesignSettings', 'AdditionalSiteDesignList', $sessionAdditionalDesignList );
         ...
         unset( $ini, $http, $sevenxAdminClassicINI, $sevenxAdminClassicSessionVarName,
             $sevenxAdminClassicAdminSiteaccess );
        // END SevenX : SevenXThemesAdminClassic : Kernel Hack
